...
internal static class LongestCommonSubsectionDiff
{
    ...
    private static IEnumerable<DiffSection> Calculate<T>(IList<T?> collection1, int lower1, int upper1, IList<T?> collection2, int lower2, int upper2, IEqualityComparer<T?> comparer, LongestCommonSubsequence<T> longestCommonSubsequence, DiffOptions options)
    {
        ...
        if (lower1 < upper1 || lower2 < upper2)
        {
            ...
            {
                if (longestCommonSubsequence.Find(lower1, upper1, lower2, upper2, out var position1, out var position2, out var length, options.ContextSize))
                {
        ...
        static IEnumerable<DiffSection> ProcessSection(IList<T?> collection1, int lower1, int upper1, IList<T?> collection2, int lower2, int upper2, IEqualityComparer<T?> comparer, LongestCommonSubsequence<T> lcs, DiffOptions options, int position1, int position2, int length)
        {
